...
//add cat points from the GeoJSON (campus_cats.js) file
	//note that we don't have to declare/create the campus_cats variable because that is done inside the campus_cats.js file.
	
L.geoJSON(campus_cats).addTo(catmap);
```
We now have many blue markers for our cats (and one for the quad that we added earlier)! 

### Style the Data
Let's do some styling and add popups to our cat markers.
